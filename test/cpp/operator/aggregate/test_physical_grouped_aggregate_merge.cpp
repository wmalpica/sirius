...
// single batch expects to return the exact same data as the input batch, since it assumes it was already aggregated
                   ...
                   "[physical_grouped_aggregate_merge]")
 {
   ...
   auto [raw_input_table, expected_table] = sirius::test::make_test_data_for_grouped_aggregate<Traits>(num_groups, 1, stream, mr);
   ...
   bool tables_match = sirius::test::expect_data_batch_equivalent_to_table(
     outputs[0], expected_table->view(), false);
...
TEMPLATE_TEST_CASE("sirius_physical_grouped_aggregate_merge grouped aggregates data_batch with multiple partition key, multiple aggregations",
  "[physical_grouped_aggregate_merge]",
  int32_t,
  int64_t,
  float,
...
auto input_tables = sirius::test::make_random_striped_split(std::move(input_table), 5, stream, mr);
...
// need to cast the expected table column 4 (which is the count column) to int64_t since at the merge stage, we end up doing a sum of int32_t which becomes an int64_t
...
auto casted_column = cudf::cast(expected_columns[4]->view(), cudf::data_type{cudf::type_id::INT64}, stream, mr);
